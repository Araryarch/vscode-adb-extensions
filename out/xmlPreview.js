...
exports.AndroidXmlPreviewPanel = void 0;
...
function parseXml(src) {
    ...
    function readUntil(chars) {
        let out = '';
        while (pos < src.length && !chars.includes(src[pos])) {
            out += src[pos++];
        }
        return out;
    }
...
function dim(v) {
    ...
    if (v === 'match_parent' || v === 'fill_parent') {
        return '100%';
    }
    if (v === 'wrap_content') {
        return 'fit-content';
...
function gravity(v, flex = true) {
    ...
    if (flex) {
        if (p.includes('center')) {
            styles.push('justify-content:center;align-items:center;');
        }
        else {
            if (p.some(x => ['center_horizontal', 'center'].includes(x))) {
                styles.push('justify-content:center;');
            }
            if (p.some(x => ['end', 'right'].includes(x))) {
                styles.push('justify-content:flex-end;');
            }
            if (p.some(x => ['center_vertical'].includes(x))) {
                styles.push('align-items:center;');
            }
            if (p.some(x => ['bottom'].includes(x))) {
                styles.push('align-items:flex-end;');
            }
        }
    }
    else {
        if (p.includes('center') || p.includes('center_horizontal')) {
...
function txt(v) {
    ...
    if (v.startsWith('@string/')) {
        return v.replace('@string/', '').replace(/_/g, ' ');
    }
    return v;
}
// ─── Node Renderer ───────────────────────────────────────────────────────────
...
// ─── HTML Shell ──────────────────────────────────────────────────────────────
function buildHtml(layoutHtml, fileName) {
...
class AndroidXmlPreviewPanel {
    static createOrShow(extensionUri, uri) {
        ...
        if (AndroidXmlPreviewPanel._current) {
            ...
            if (uri) {
                AndroidXmlPreviewPanel._current._update(uri);
            }
            return;
        }
        const panel = vscode.window.createWebviewPanel(AndroidXmlPreviewPanel.viewType, 'Layout Preview', col, { enableScripts: true, localResourceRoots: [extensionUri] });
...
exports.AndroidXmlPreviewPanel = AndroidXmlPreviewPanel;
AndroidXmlPreviewPanel.viewType = 'droidStudio.xmlPreview';
//# sourceMappingURL=xmlPreview.js.map